...
if __name__ == "__main__":
    logger.info("--- Ollama LLM Interface Example ---")

    # Make sure Ollama is running and has pulled the model (e.g., `ollama run mistral`)
    try:
        # Initialize with default settings
        # Replace DEFAULT_MODEL with the actual model you have pulled if different
        llm = OllamaLLM(model_name=DEFAULT_MODEL)

        # Example 1: Simple generation (non-streaming)
        print("\n--- Example 1: Simple Generation ---")
        simple_prompt = "Why is the sky blue?"
        print(f"Prompt: {simple_prompt}")
        try:
            start_time = time.time()
            response_text = llm.generate(simple_prompt, stream=False)
            end_time = time.time()
            print(f"\nResponse (took {end_time - start_time:.2f}s):\n{response_text}")
        except Exception as e:
            print(f"\nError during simple generation: {e}")

        # Example 2: Generation with options and system prompt (non-streaming)
        print("\n--- Example 2: Generation with Options & System Prompt ---")
        options_prompt = "Write a short haiku about servers."
        system = "You are a poetic assistant."
        gen_options = {"temperature": 0.6, "seed": 42} # Add seed for reproducibility if supported
        print(f"System: {system}")
        print(f"Prompt: {options_prompt}")
        print(f"Options: {gen_options}")
        try:
            start_time = time.time()
            response_text = llm.generate(options_prompt, system_prompt=system, options=gen_options, stream=False)
            end_time = time.time()
            print(f"\nResponse (took {end_time - start_time:.2f}s):\n{response_text}")
        except Exception as e:
            print(f"\nError during generation with options: {e}")


        # Example 3: Streaming generation
        print("\n--- Example 3: Streaming Generation ---")
        stream_prompt = "Explain the concept of RAG (Retrieval-Augmented Generation) in 3 sentences."
        print(f"Prompt: {stream_prompt}")
        print("\nResponse (Streaming):")
        try:
            start_time = time.time()
            response_stream = llm.generate(stream_prompt, stream=True)
            full_streamed_response = ""
            for chunk in response_stream:
                print(chunk, end='', flush=True) # Print chunks as they arrive
                full_streamed_response += chunk
            end_time = time.time()
            print(f"\n\nStream finished (took {end_time - start_time:.2f}s).")
            # print(f"Full streamed response:\n{full_streamed_response}") # Optionally print the combined result
        except Exception as e:
            print(f"\nError during streaming generation: {e}")


    except ConnectionError as e:
        print(f"\nConnection Error: Could not connect to Ollama. Please ensure it's running.")
    except Exception as e:
         print(f"\nAn unexpected error occurred: {e}")

    logger.info("--- Ollama LLM Interface Example End ---")