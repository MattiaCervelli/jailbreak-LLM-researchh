...
def get_data(
    ...
    return final_data # Type: List[Dict[str, Any]]
...
if __name__ == "__main__":
    ...
    # Basic loading example (adjust dataset names based on your actual subdirs)
    available_datasets = [d.name for d in PROCESSED_DATA_DIR.iterdir() if d.is_dir()]
    print(f"Available dataset folders found: {available_datasets}")
    print("-" * 30)

    if not available_datasets:
        print("No dataset folders found in data/processed/. Cannot run examples.")
        print("Please ensure you have subfolders like 'anthropic', 'advbench', 'jbb' etc.")
        print("containing your standardized .jsonl files.")
    else:
        # Example 1: Load all data from one dataset
        try:
            target_dataset = available_datasets[0] # Use the first available dataset
            print(f"\n[Example 1: Load all data from '{target_dataset}']")
            all_data = get_data(target_dataset)
            print(f"Loaded {len(all_data)} records from '{target_dataset}'. First record:")
            if all_data:
                print(all_data[0])
            else:
                print("No data loaded (check file contents or path).")
        except FileNotFoundError as e:
            print(f"Error loading {target_dataset}: {e}")
        except Exception as e:
            print(f"An unexpected error occurred during Example 1: {e}")

        print("-" * 30)

        # Example 2: Load data from multiple datasets with filtering
        # Adjust filter_criteria based on actual keys in your standardized JSONL
        # Assuming a common key 'category' or 'technique' might exist
        # Check one record to guess a key for filtering/balancing demonstration
        filter_key_guess = None
        balance_key_guess = None
        if all_data:
            sample_record = all_data[0]
            # Try common keys expected in jailbreaking datasets
            for key in ['technique', 'attack_type', 'category', 'goal', 'behavior_id', 'dataset_source']:
                 if key in sample_record:
                     filter_key_guess = key
                     # Try to find a key with multiple potential values for balancing
                     if isinstance(sample_record[key], str) and len(set(d.get(key) for d in all_data if d.get(key))) > 1:
                         balance_key_guess = key
                         break # Found a suitable key for balancing
                     elif balance_key_guess is None: # Keep the first potential key found
                         balance_key_guess = key


        if filter_key_guess and all_data:
            # Find a specific value present in the data for filtering
            filter_value_guess = all_data[0].get(filter_key_guess)
            if filter_value_guess:
                print(f"\n[Example 2: Load from '{target_dataset}', filter by '{filter_key_guess}' == '{filter_value_guess}']")
                try:
                    filtered_data = get_data(
                        target_dataset,
                        filter_criteria={filter_key_guess: filter_value_guess}
                    )
                    print(f"Loaded {len(filtered_data)} records matching the filter.")
                    if filtered_data:
                        print("First matching record:", filtered_data[0])

                        # Example 3: Random sampling
                        print(f"\n[Example 3: Randomly sample 5 records from the filtered set]")
                        sampled_random = get_data(
                            target_dataset,
                            n_samples=5,
                            filter_criteria={filter_key_guess: filter_value_guess},
                            random_seed=42
                        )
                        print(f"Sampled {len(sampled_random)} records randomly.")
                        # for record in sampled_random: print(record) # Uncomment to see samples

                        # Example 4: Balanced sampling (if a suitable key was found)
                        if balance_key_guess:
                            print(f"\n[Example 4: Sample 10 records, balanced by '{balance_key_guess}']")
                            # Load more data for better balancing demo if needed
                            all_data_for_balancing = get_data(target_dataset) # Reload without filter
                            sampled_balanced = get_data(
                                target_dataset,
                                n_samples=10,
                                balance_key=balance_key_guess,
                                random_seed=42
                            )
                            print(f"Sampled {len(sampled_balanced)} records with balancing attempt.")
                            if sampled_balanced:
                                counts = defaultdict(int)
                                for record in sampled_balanced:
                                    counts[record.get(balance_key_guess)] += 1
                                print("Counts per group in balanced sample:", dict(counts))
                        else:
                            print(f"\n[Example 4: Skipped - Could not determine a suitable key like 'technique' or 'category' for balanced sampling example]")

                except Exception as e:
                    print(f"An unexpected error occurred during Example 2/3/4: {e}")
            else:
                 print(f"\n[Example 2/3/4: Skipped - Could not determine a value for the guessed key '{filter_key_guess}'")
        else:
             print(f"\n[Example 2/3/4: Skipped - Could not guess a suitable key ('technique', 'category', etc.) from the first record for filtering/balancing examples, or no data was loaded.]")


        # Example 5: Lazy Loading (demonstrates iteration)
        print(f"\n[Example 5: Lazy load from '{target_dataset}' with filter]")
        try:
             # Use the filter from before if available
             lazy_filter = {filter_key_guess: filter_value_guess} if filter_key_guess and filter_value_guess else None
             print(f"Using filter: {lazy_filter}")
             lazy_gen = get_data(target_dataset, filter_criteria=lazy_filter, lazy=True)
             print("Iterating through first 3 records from lazy generator:")
             count = 0
             for record in lazy_gen:
                 print(record)
                 count += 1
                 if count >= 3:
                     break
             print(f"Successfully iterated through {count} records.")
             # Consume the rest of the generator to trigger final log messages if any
             # list(lazy_gen) # Uncomment if you want to see the final count log message
        except FileNotFoundError as e:
            print(f"Error during lazy loading: {e}")
        except ValueError as e:
             print(f"Configuration error during lazy loading: {e}")
        except Exception as e:
            print(f"An unexpected error occurred during Example 5: {e}")